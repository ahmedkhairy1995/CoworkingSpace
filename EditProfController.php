...
if (isset($_SESSION['views'])) {
    if (isset($_POST["SaveProfile"])) {
        ...
        if (!$validationController->validateEmail($Email)) {
        ...
        } else if (!$validationController->validatePassword($CurrentPassword)) {
            redirect_to("EditProfile.php?flag=6");
        } else if (!$validationController->validateAddress($Address)) {
            redirect_to("EditProfile.php?flag=7");
        } else if (($NewPassword != "" && !$validationController->validatePassword($NewPassword)) || ($ConfirmNewPassword != "" && !$validationController->validatePassword($ConfirmNewPassword))){
            redirect_to("EditProfile.php?flag=6");
        }
        else {
            ...
            if ($actualPassword != md5($CurrentPassword))
                ...
                if ($NewPassword == "" && $ConfirmNewPassword == "") {
                    $result = $userController->updateProfile($user->getId(), $FirstName, $LastName, $user->getPassword(), $Email, $MobileNumber, $Address);
                    if ($result) {
                    ...
                    $result = $userController->updateProfile($user->getId(), $FirstName, $LastName, md5($NewPassword), $Email, $MobileNumber, $Address);
